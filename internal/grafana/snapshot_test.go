	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if snapExists != tt.expectSnapshotToExist {
				t.Errorf("GetSnapshot() exists check snapExists = %v, want %v", snapExists, tt.expectSnapshotToExist)
			}
			if !tt.expectSnapshotToExist{
				if !reflect.DeepEqual(snapshot, tt.expectedSnapshot) {
					t.Errorf("GetSnapshot() snapshot = %v, want %v", snapshot, tt.expectedSnapshot)
				}
			}
		})
	}
